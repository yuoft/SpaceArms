...
public enum MyArmorMaterial implements IArmorMaterial {
	...
	TOTEM(Spacearms.MOD_ID + ":" + "totem", 20, new int[] { 3, 6, 8, 3 }, 15, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 2.5F, 0,() -> {
		return Ingredient.fromItems(Items.TOTEM_OF_UNDYING);
	}),
	GLOWSTONE(Spacearms.MOD_ID + ":" + "glowstone", 15, new int[] { 2, 5, 6, 2 }, 8, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 0,0, () -> {
		return Ingredient.fromItems(Items.GLOWSTONE);
	}),
	RUBY(Spacearms.MOD_ID + ":" + "ruby", 35, new int[] { 4, 7, 9, 4 }, 10, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 2F, 0,() -> {
		return Ingredient.fromItems(SAItems.rubyIngot.get());
	}),
	EMERALD(Spacearms.MOD_ID + ":" + "emerald", 39, new int[] { 5, 8, 10, 5 }, 11, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 2.5F, 0,() -> {
		return Ingredient.fromItems(SAItems.emeraldIngot.get());
	}),
	XRAY(Spacearms.MOD_ID + ":" + "xray", 45, new int[] { 6, 9, 11, 6 }, 12, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 2F, 0,() -> {
		return Ingredient.fromItems(SAItems.xrayIngot.get());
	}),
	SUPER(Spacearms.MOD_ID + ":" + "super", 49, new int[] { 8, 11, 13, 8 }, 13, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 3F, 0,() -> {
		return Ingredient.fromItems(SAItems.superIngot.get());
	}),
	DRAGON(Spacearms.MOD_ID + ":" + "dragon", 53, new int[] { 9, 12, 14, 9 }, 15, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 3.5F, 0.1f,() -> {
		return Ingredient.fromItems(SAItems.dragonCrystal.get());
	}),
	SUPER_XRAY(Spacearms.MOD_ID + ":" + "xray", 58, new int[] { 11, 14, 16, 11 }, 15, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 3.5F, 0.05f,() -> {
		return Ingredient.fromItems(SAItems.superXrayIngot.get());
	}),
	ULTRA(Spacearms.MOD_ID + ":" + "ultra", 65, new int[] { 13, 16, 18, 13 }, 16, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 4F, 0.15f,() -> {
		return Ingredient.fromItems(SAItems.ultraIngot.get());
	}),
	SPACE(Spacearms.MOD_ID + ":" + "space", 99, new int[] { 16, 20, 25, 16 }, 30, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 5.0F, 0.3f,() -> {
